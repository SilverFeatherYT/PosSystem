            ...
            <div class="bot">
                <div id="table">
                    <div class="form-group datepicker-block label-sticky">
                        <label>Date: {{ $invoice->created_at->format('Y-m-d') }}</label>
                    </div>
                    <div class="form-group datepicker-block label-sticky">
                        <label>Customer Name: {{ $receiptsData[$invoice->D_RecID][0]->cusName }}</label>
                    </div>
                    <div class="form-group datepicker-block label-sticky">
                        <label>Pay By: {{ $receiptsData[$invoice->D_RecID][0]->payment }}</label>
                    </div>
                    <table>
                        @csrf
                        <thead>
                        ...
                        <tbody>
                            @foreach ($receiptsData[$invoice->D_RecID] as $receipt)
                            <tr>
